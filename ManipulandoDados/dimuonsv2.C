...
void dimuons()
{
  ...
  a._mmin = 75.0;
  a._mmax = 105.0;
  extraString = "Z"; 	// NOTE: this is used in file name:
...
void dimuon::FitPeakRoofit()
{
  ...
  // Define background model (Poisson) and its parameter
  RooRealVar mean("mean","mean",0.5*(_mmin+_mmax),_mmin,_mmax);
  RooPoisson background("background","background",mass,mean);
  
  
  // Define signal model (CrystalBall) and its parameters
  RooRealVar sigma_cb("sigma_cb","sigma_cb", 2.5);
  RooRealVar alpha("alpha", "alpha", 0.8);
  RooRealVar n("n", "n", 11);
  RooCBShape crystalball("CB", "CB", mass, mean, sigma_cb, alpha, n);
  

  // Define signal model (BreitWigner) and its parameters
  RooRealVar sigma("sigma","sigma",0.6*(_mmax-_mmin),0.,0.9*(_mmax-_mmin));
  RooRealVar width("width","width",3.4);
  RooBreitWigner breitwigner("BW","BW",mass,mean,width);
  ...
  RooAddPdf* signal;
  signal = new RooAddPdf("signal", "signal", RooArgList(breitwigner, crystalball), RooArgList(frac1));
  ...
  model->plotOn(frame,Name("modelCB"),Components("CB"),LineStyle(kDashed),LineColor(kYellow)) ;
  model->plotOn(frame,Name("modelBW"),Components("BW"),LineStyle(kDashed),LineColor(kMagenta)) ;
  ...
  legend->AddEntry(frame->findObject("modelCB"),"Crystal Ball","l");
  legend->AddEntry(frame->findObject("modelBW"),"BreitWigner","l");
