...
void dimuon::FitPeakRoofit()
{
  ...
  // Define background model (polynomial) and its parameter
  RooRealVar a0("a0", "a0", 0, -10, 10);
  RooRealVar a1("a1", "a1", 0, -10, 10);
  RooPolynomial background("background", "background", mass, RooArgList(a0,a1));
  
  
  // Define signal model (voitgtian) and its parameters
  ...
  RooVoigtian voitgtian("VT","VT",mass,mean,width,sigma);
  ...
  model->plotOn(frame,Name("modelCB"),Components("CB"),LineStyle(kDashed),LineColor(kGreen)) ;
  model->plotOn(frame,Name("modelVT"),Components("VT"),LineStyle(kDashed),LineColor(kOrange)) ;
  ...
  legend->AddEntry(frame->findObject("modelVT"),"Voitgtian","l");
  legend->Draw();
